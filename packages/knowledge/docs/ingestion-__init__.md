...
generated_at: 2026-02-01T19:33:42.003825
...
2. **`build_or_update_index`**:
   ...
   - **Behavior:** It processes the provided document chunks and adds them to an existing index or creates a new index if one does not exist. The function handles the details of index creation, storage, and optimization.
...
- **Exposed API:** The `__all__` list explicitly defines the public API of the package, controlling which functions are accessible to external users. This helps to maintain a clean and stable interface.
...
The functionality of this package relies on the implementations within the `federated_loader` and `index_builder` modules. You should consult the documentation for those modules for more detailed information about their specific dependencies and configurations.