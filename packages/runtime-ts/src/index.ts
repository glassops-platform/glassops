...
import { initTelemetry, withSpan, shutdown, addSpanEvent } from "./services/telemetry";
import {
...
/**
 * Helper to get input from either GitHub Actions core or environment variables
...
function getInput(name: string, defaultValue?: string): string {
  // Try GitHub Actions core first
  const coreInput = core.getInput(name);
  if (coreInput) return coreInput;

  // Try Docker environment variable (INPUT_<NAME> format)
  const envName = `INPUT_${name.toUpperCase()}`;
  const envValue = process.env[envName];
  if (envValue) return envValue;

  // Try GLASSOPS_ prefixed environment variable
  const glassopsEnvName = `GLASSOPS_${name.toUpperCase()}`;
  const glassopsEnvValue = process.env[glassopsEnvName];
  if (glassopsEnvValue) return glassopsEnvValue;
...
async function run(): Promise<void> {
  try {
    ...
    const jwtKey = getInput("jwt_key");
    if (!jwtKey.includes("BEGIN") || !jwtKey.includes("END")) {
      throw new Error(
        "Invalid JWT key format - must contain BEGIN and END markers",
      );
    }
    // Mask JWT key in all logs
    core.setSecret(jwtKey);

    // Validate Salesforce instance URL
    const instanceUrl = getInput("instance_url", "https://login.salesforce.com");
    try {
    ...
    // Initialize OpenTelemetry (no-op if endpoint not configured)
    await initTelemetry(
      getInput("otel_service_name") || "glassops-runtime",
      "1.0.0",
    );

    // 0. Cache Retrieval Phase (Note: Cache may not work the same in Docker)
    // Docker containers are ephemeral, so cache strategy needs adjustment
    core.startGroup("📦 Checking Runtime Cache");
    try {
      const cacheKey = `glassops-runtime-${process.platform}-${process.arch}-${process.version}`;
      const cachePaths = [
        path.join(process.env.HOME || "/home/glassops", ".npm"),
        path.join(process.env.HOME || "/home/glassops", ".sf"),
      ];
      ...
      if (cacheHit) {
        ...
        core.info("⚠️ Cache miss - will proceed without cache");
      }
    } catch (error) {
      log.warning("Cache operations not available in Docker context, skipping...", "Cache");
    }
    core.endGroup();
    ...
    try {
      ...
      if (config.governance.analyzer?.enabled) {
        ...
        const scanResults = await analyzer.scan(
          ["."],
          config.governance.analyzer.rulesets?.[0],
        );

        const criticalViolations = scanResults.violations.filter(
          (v) =>
            v.severity <= (config.governance.analyzer?.severity_threshold || 1),
        );

        if (criticalViolations.length > 0) {
          throw new PolicyError(
            `Static analysis failed: ${criticalViolations.length} critical violations found.`,
          );
        }
        log.info("✅ Static analysis passed", "Analyzer");
      }

      if (getInput("enforce_policy") === "true") {
        try {
    ...
    // 2. Bootstrap Phase
    await withSpan("glassops.bootstrap", async (span) => {
      ...
      try {
        ...
        span.setAttribute("cli_version", config.runtime.cli_version);
        log.info(
          `✅ CLI ${config.runtime.cli_version} installed successfully`,
          "Bootstrap",
        );

        // Verify CLI health after install
        const health = await healthCheck();
        if (!health.healthy) {
          throw new BootstrapError(`CLI health check failed: ${health.error}`);
        }
        addSpanEvent("health_check_passed", { version: health.version || "unknown" });
    ...
    // 3. Identity Phase
    let orgId: string = await withSpan("glassops.identity", async (span) => {
      log.info("Authenticating with Salesforce...", "Identity");

      if (getInput("skip_auth") === "true") {
        log.warning(
          "⚠️ Skipping authentication as requested by configuration",
          "Identity",
        );
        span.setAttribute("skipped", true);
        return "00D00000000TEST"; // Dummy Org ID for testing
      }

      try {
        const identity = new IdentityResolver();
        const authenticatedOrgId = await identity.authenticate({
          clientId: getInput("client_id"),
          jwtKey: getInput("jwt_key"),
          username: getInput("username"),
          instanceUrl: getInput("instance_url"),
        });
        span.setAttribute("org_id", authenticatedOrgId);
        log.info(`✅ Authenticated with org ${authenticatedOrgId}`, "Identity");
    ...
    try {
      ...
      const testResultsInput = getInput("test_results");
      if (testResultsInput) {
        try {
      ...
      const coverageActual =
        parseFloat(getInput("coverage_percentage")) || 0;
      const coverageRequired =
        parseFloat(getInput("coverage_required", "80")) || 80;
