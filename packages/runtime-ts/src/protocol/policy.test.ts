...
describe("ProtocolPolicy", () => {
  ...
  describe("load", () => {
    ...
    it("should look for config in the config/ directory", async () => {
      mockedExistsSync.mockReturnValue(false);
      await policy.load();
      // Verify it called verify with a path containing config/devops-config.json
      // We check the first argument of the first call to existsSync
      const callArgs = mockedExistsSync.mock.calls[0][0];
      expect(callArgs).toContain("config");
      expect(callArgs).toContain("devops-config.json");
    });
